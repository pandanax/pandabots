# n8n Workflows

Эта папка содержит JSON файлы готовых workflow для импорта в n8n.

## 📋 Список workflow

### 1. `telegram-hello-bot.json`
**Описание:** Простой Telegram бот, который отвечает "Здарова!" на любое сообщение  
**Требуемые credentials:** Telegram Bot API Token  
**Инструкция:** [SETUP_TELEGRAM_BOT.md](SETUP_TELEGRAM_BOT.md)

### 2. `telegram-deepseek-bot.json`
**Описание:** AI-powered Telegram бот с DeepSeek V3  
**Возможности:**
- Умный диалог на русском языке
- ✅ **Работает из России** (нет блокировок)
- Очень дешево: $0.27/$1.10 за 1M токенов
- Качество близко к GPT-4
- Настраиваемая личность бота
- Поддержка Markdown в ответах

**Требуемые credentials:**
- Telegram Bot API Token
- DeepSeek API Key

**Инструкция:** [SETUP_DEEPSEEK.md](SETUP_DEEPSEEK.md)

### 3. `mandala-bot-advanced.json` 🌟 ADVANCED!
**Описание:** Продвинутый бот-специалист по персональным мандалам  
**Возможности:**
- 🎨 **Специализация:** Эксперт по созданию персональных мандал
- 💬 **История диалога:** Помнит контекст для каждого пользователя (PostgreSQL)
- 📚 **RAG:** Использует базу знаний о мандалах (`rags/mandala-bot-advanced`)
- 🧮 **Расчеты:** Нумерологические мандалы по дате рождения
- 📖 **Инструкции:** Пошаговые гайды как рисовать мандалу
- 👥 **Multi-user:** Отдельная история для каждого пользователя

**Требуемые credentials:**
- Telegram Bot API Token (MandalaBot)
- DeepSeek API Key
- PostgreSQL credentials

**Требуемая инфраструктура:**
- PostgreSQL таблица `chat_history`
- База знаний в `rags/mandala-bot-advanced/`

**Инструкция:** [SETUP_MANDALA_BOT.md](SETUP_MANDALA_BOT.md)

### 4. `telegram-fitbot.json` 🌟 ADVANCED!
**Описание:** Профессиональный консультант по здоровому питанию и коррекции пищевого поведения  
**Возможности:**
- 🍎 **Специализация:** Эксперт по питанию и физиологии похудения
- 💬 **История диалога:** Помнит контекст для каждого пользователя (PostgreSQL)
- 📚 **RAG:** Научная база знаний о питании (`rags/fitbot`)
- 📊 **Персонализация:** Индивидуальные планы питания
- 🔬 **Научный подход:** Только доказанные методы
- 🎯 **Целеполагание:** Помощь в достижении целей по весу
- 👥 **Multi-user:** Отдельная история для каждого пользователя

**Требуемые credentials:**
- Telegram Bot API Token (FitBot)
- DeepSeek API Key
- PostgreSQL credentials

**Требуемая инфраструктура:**
- PostgreSQL таблица `chat_history`
- База знаний в `rags/fitbot/`

**Инструкция:** [SETUP_FITBOT.md](SETUP_FITBOT.md)

---

## 🔧 Как использовать

### Импорт workflow в n8n:

1. Открой n8n: https://n8n.mandala-app.online
2. Нажми "Add workflow" → "Import from File"
3. Выбери JSON файл из этой папки
4. Настрой credentials (см. ниже)

### Настройка Telegram Bot credentials:

1. В n8n перейди: **Settings** → **Credentials** → **Add Credential**
2. Выбери тип: **Telegram API**
3. Введи данные:
   - **Credential Name:** `Telegram Test Bot` (или любое имя)
   - **Access Token:** Токен от @BotFather
4. Нажми **Save**
5. В workflow выбери этот credential в узлах Telegram

### Webhook URL для Telegram:

После импорта workflow:
1. Скопируй Production Webhook URL из узла "Telegram Trigger"
2. Он будет вида: `https://n8n.mandala-app.online/webhook/telegram-test-bot`
3. n8n автоматически зарегистрирует webhook у Telegram

---

## 🔒 Безопасность

⚠️ **НИКОГДА не коммить файлы с токенами!**

- ✅ JSON workflow (без токенов) - можно в git
- ❌ Токены ботов - только в n8n credentials
- ❌ `.env` файлы с токенами - только локально

---

## 📂 Структура файлов

```
workflows/
├── README.md                      # Этот файл
├── telegram-hello-bot.json        # Простой Telegram бот
├── telegram-deepseek-bot.json     # AI бот с DeepSeek
├── mandala-bot-advanced.json      # Бот-эксперт по мандалам (с историей и RAG)
├── telegram-fitbot.json           # Консультант по питанию (с историей и RAG)
├── SETUP_TELEGRAM_BOT.md          # Инструкция: простой бот
├── SETUP_DEEPSEEK.md              # Инструкция: DeepSeek бот
├── SETUP_MANDALA_BOT.md           # Инструкция: Mandala Bot
└── SETUP_FITBOT.md                # Инструкция: FitBot
```

---

## 💡 Полезные ссылки

- **n8n Docs:** https://docs.n8n.io/
- **Telegram Bot API:** https://core.telegram.org/bots/api
- **Создать бота:** https://t.me/BotFather
